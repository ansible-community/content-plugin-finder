...
content-plugin-finder --parent ../ansible.platform --jobs 8
```
...
CLI directory scans use up to four processes by default. Use `--jobs N` to tune
the parallelism for the available CPU and memory, or `--jobs 1` for serial
execution. Library calls through `Orchestrator.scan()` remain serial unless
`jobs` is explicitly set.
